...
%% Compute the front and rear cornerning stiffness
Ca_r= Fz_r * B * C;
Ca_f= Fz_f * B * C;

%% 2.1.1 Generate equilibrium trajetory using Euler integration and linear tire forces
delta_fun=@(t) (10*pi/180)*sin(2*pi*t)-0.00175;
...
%% 2.1.2 Linearize bicycle model for feedback gains with linear tire forces
phi_eq = Z_eq(3,:);
v_eq = Z_eq(4,:);
...
tireforce_percent_error= 100 * max(e_front,e_rear)
...
title('Extreme: Tire Forces')
hold on
plot(<'front tire slip angle (linear)'>, <'front lateral tire force (linear)'>)
plot(<'front tire slip angle (nonlinear)'>, <'front lateral tire force (nonlinear)'>)
legend('linear tire forces','Nonlinear (actual) tire forces')
subplot(2,1,2)
hold on
plot(<'rear tire slip angle (linear)'>, <'rear lateral tire force (linear)'>)
plot(<'rear tire slip angle (nonlinear)'>, <'rear lateral tire force (nonlinear)'>)
legend('linear tire forces','Nonlinear (actual) tire forces')
%}
...
%% 2.1.4 Euler intergrate bicycle model with nonlinear tire dynamics
...
% dis_square = (Z_eq(1,:) - Z_eq_nl(1,:)).^2 + (Z_eq(1,:) - Z_eq_nl(1,:)).^2
